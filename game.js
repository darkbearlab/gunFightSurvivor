...
function openSettings() {
    ...
    content.innerHTML = '';
    const settingsMap = [
        { title: "宏觀規則", items: [{ key: 'gameDuration', label: '時間(秒)' }, { key: 'targetRep', label: '目標名聲' }, { key: 'initialGold', label: '初始金' }] },
        { title: "經濟平衡", items: [{ key: 'costSoldier', label: '步兵價' }, { key: 'costArcher', label: '弓兵價' }, { key: 'dropRate', label: '掉寶率' }] },
        { title: "難度", items: [{ key: 'patrolBaseInterval', label: '巡邏間隔' }] }
    ];
...
function toggleMode() {
    ...
    if (isAutoAttackMode) { btn.innerHTML = "⚔️<br>攻擊"; btn.style.backgroundColor = "rgba(255, 0, 0, 0.6)"; } 
    else { btn.innerHTML = "🛡️<br>跟隨"; btn.style.backgroundColor = "rgba(0, 255, 0, 0.5)"; }
}
...
function checkGameOver() { if(isGameOver)return; if(gameTimeLeft<=0) { if(reputation>=G.targetRep) endGame(true, "傳奇誕生！<br>你成功抵抗了暴政。"); else endGame(false, "起義失敗。<br>歷史遺忘了你的名字。"); } }
...
class Loot extends Phaser.GameObjects.Text {
    constructor(scene, x, y, type) {
        let icon = '💰'; if (type === 'heal') icon = '❤️'; else if (type === 'buff') icon = '⚔️';
    ...
    pickup() {
        ...
        else if (this.type === 'buff') { player.activateBuff(); showToast(this.scene, "攻擊力提升!", "red"); }
        this.destroy();
    }
}
...
class Hero extends Phaser.Physics.Arcade.Sprite {
    ...
    initClassStats() {
        ...
        if (this.classType === 'warrior') {
            this.maxHp = 450;
            this.hp = 450;
            this.attackCooldown = 300; // 攻速快
            this.detectRange = 80;     // 距離短
            this.speed = G.heroSpeed;
        } else if (this.classType === 'archer') {
            this.maxHp = 200;
            this.hp = 200;
            this.attackCooldown = 600; // 攻速慢
            this.detectRange = 300;    // 距離長
            this.speed = G.heroSpeed * 1.1; // 跑快一點點
        } else if (this.classType === 'mage') {
            this.maxHp = 150;
            this.hp = 150;
            this.attackCooldown = 500;
            this.detectRange = 180;    // 中距離
            this.speed = G.heroSpeed;
        }
    }
        ...
        if (Date.now() - this.lastAttackTime > this.attackCooldown) {
            ...
            if (target) { 
                ...
                if (this.classType === 'warrior') {
                    ...
                    projectiles.add(new Projectile(this.scene, this.x, this.y, target, dmg * 1.2, false, 350, 1000, 0x00ffff)); // 法師球飛慢點但痛
                }
    ...
    die() { this.isDead = true; this.hpBar.destroy(); this.hpBarBg.destroy(); this.destroy(); endGame(false, "你戰死沙場，革命之火熄滅了。"); }
}
...
class Carriage extends EnemyUnit {
    ...
    die() { updateGold(600); updateRep(150); showToast(this.scene, "搶得 $600 + 150 Rep!", "gold"); super.die(); }
}
...
class Manor {
    ...
    destroyManor() {
        ...
        this.label.setText("已摧毀"); this.label.setColor("#aaa");
        if (this.type === 'gold') { permanentBuffs.gold = true; document.getElementById('buff-gold').style.display = 'inline'; showToast(this.scene, "獲得黃金聖杯！(每秒+$)", "gold"); } else { permanentBuffs.rep = true; document.getElementById('buff-rep').style.display = 'inline'; showToast(this.scene, "獲得貴族印章！(每秒+Rep)", "cyan"); }
...
class WanderingChampion extends Unit {
    ...
    die() {
        if (!this.isRecruited) {
            ...
            showToast(this.scene, "劍聖加入了你的隊伍！", "gold");
        } else {
            super.die();
        }
    }
...
class Village {
    constructor(scene, x, y, name) {
        ...
        this.zones = { upgrade: { x: x, y: y - 60, r: 30, color: 0xffff00, timer: 0, required: 3000, action: 'upgrade', label: '升級' }, soldier: { x: x - 50, y: y + 50, r: 30, color: 0x0000ff, timer: 0, required: 1000, action: 'recruit_h', label: '步兵' }, archer: { x: x + 50, y: y + 50, r: 30, color: 0x00ffff, timer: 0, required: 1000, action: 'recruit_j', label: '弓手' } };
    ...
    upgrade() { 
        ...
        if (gold >= cost) { 
            ...
            this.refreshVisuals(); showToast(this.scene, `${this.name} 升級至 Lv.${this.level}!`, "cyan"); 
            if (this.level === 5) {
                ...
                showToast(this.scene, "駐守衛隊已部署！", "gold");
            }
            return true; 
        } 
        return false; 
    }
    downgrade() { this.level = 1; this.integrity = 100; updateRep(-200); showToast(this.scene, `${this.name} 被攻陷了！`, "red"); this.refreshVisuals(); }
    recruit(type) { 
        if (this.stock <= 0) return false; 
        let cost = (type === 'H') ? G.costSoldier : G.costArcher; 
        if (gold >= cost) { updateGold(-cost); this.stock--; this.refreshVisuals(); showToast(this.scene, `招募成功 (-$${cost})`, "lime"); return true; } return false; 
    }
...
class GameDirector {
    ...
    spawnPatrol(difficulty) {
        ...
        let msg = (target === player) ? "巡邏隊正在搜捕你！" : `巡邏隊正在前往 ${target.name}！`; showToast(this.scene, msg, "red");
...
function create() {
    ...
    villages = [ new Village(this, 400, 400, "北境村"), new Village(this, 1000, 1000, "中央市集"), new Village(this, 1600, 1600, "南風鎮") ];
